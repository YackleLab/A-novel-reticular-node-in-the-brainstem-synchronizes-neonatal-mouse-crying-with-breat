...
firstsylindex = [1 diff(vocExpStart)] ~= 0;
...
allCallLens = (twhis(2,:)-twhis(1,:))*1000;
meanallCallLens = mean(allCallLens);


uniCallLens = allCallLens (singlesyllabindex);
meanuniCallLens = mean (uniCallLens);
numUniCalls = length (uniCallLens); 

biCallLens = allCallLens (bisyllabindex);
biCallLens_firstsyl = biCallLens(1:2:end);
biCallLens_secondsyl = biCallLens(2:2:end);
meanbiCallLens = mean (biCallLens);
numBiCalls = length (biCallLens); 

triCallLens = allCallLens (trisyllabindex);
triCallLens_firstsyl = triCallLens(1:3:end);
triCallLens_secondsyl = triCallLens(2:3:end);
triCallLens_thirdsyl = triCallLens(3:3:end);
meantriCallLens = mean (triCallLens);
numTriCalls = length (triCallLens); 

firstSylCallLens = (twhis(2,firstsylindex)-twhis(1,firstsylindex))*1000;

save (fullfile(pathtodata,strcat(filename,'callLens_2.mat')),...
    'numcalls','numMScalls','numUniCalls','numBiCalls','numTriCalls',... 
    'allCallLens','meanallCallLens',...
    'uniCallLens','meanuniCallLens',...
    'biCallLens','biCallLens_firstsyl','biCallLens_secondsyl','meanbiCallLens',...
    'triCallLens','triCallLens_firstsyl','triCallLens_secondsyl','triCallLens_thirdsyl','meantriCallLens',...
    'firstSylCallLens');
